...
## この案件は何か
RAMUSEのコンテスト管理アプリ（大人用）。GAS Webアプリ。子供用は ../ramuse-kid。共有スプレッドシートで連携。

## アカウント（重要）
- **この案件だけ GitHub は ramuse-lock アカウント**（git@github-ramuse: 経由・コミット名義も自動でRAMUSE用に切替）
- GAS側もRAMUSE用のGoogleアカウント（claspの認証切替に注意）

## 既知の課題
- GASがクロスオリジンiframeをブロック→バナー/Safari UI問題。真の解決はAPI化（fetch）だが未着手
...
編集後は必ず以下の4ステップをすべて完了させること。GitHubに存在しない状態を本番へ出さないため、**コミット・pushを先に行う**：
...
```bash
cd /Users/chisato/personal/ramuse/ramuse-kid
...
| 変更対象 | 作業リポジトリ | コマンド |
|---|---|---|
| 親アプリ HTML または GAS バックエンド | `ramuse-deploy` | `git commit` → `git push` → `clasp push` → `clasp deploy` → 本番確認 |
| 子供用アプリ HTML のみ | `ramuse-kid` | `git push`（GAS には触らない） |
| ステータスロジック・バッジ・結果表示など共通仕様 | **両方** | ramuse-deploy: 上記4ステップ / ramuse-kid: `git push` |
...
| `Setup.gs` | 初期設定用スクリプト |
| `/Users/chisato/personal/ramuse/ramuse-kid/index.html` | 子供用アプリのメイン HTML（GitHub Pages） |
